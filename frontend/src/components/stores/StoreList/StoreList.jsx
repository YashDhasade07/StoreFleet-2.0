...
import RatingForm from '../../ratings/RatingForm/RatingForm.jsx';
...
const StoreList = ({ showActions = true, allowRating = false }) => {
    ...
    const handleRateStore = (store) => {
        console.log('handleRateStore called for:', store.name);
        setSelectedStoreForRating(store);
        setEditingRating(null);
        setShowRatingForm(true);
    };

    const handleUpdateRating = async (store) => {
        console.log('handleUpdateRating called for:', store.name);
        
        if (!store.userRating) {
          showError('You have not rated this store yet.');
          return;
        }
      
        try {
          // Fetch the full rating data including ID
          const result = await ratingService.getUserRatingForStore(store.id);
          ...
          if (result.success) {
            ...
            showError('Could not find your rating for this store.');
          }
        } catch (error) {
          showError('Error loading your rating: ' + error.message);
        }
      };
        ...
        try {
            ...
            if (editingRating) {
                ...
                result = await ratingService.submitRating({
                    ...ratingData,
                    store_id: selectedStoreForRating.id
                });
            }
                            ...
                            <div className="store-grid">
                                {stores.map((store) => (
                                    <StoreCard
                                        ...
                                        onRate={handleRateStore}           // Add this
                                        onUpdateRating={handleUpdateRating} // Add this
